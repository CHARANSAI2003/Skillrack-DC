The program must accept N queries as the input. Each query contains a character CH and an integer X. The program must form a list based on the queries. The program must
print the output by executing the queries one by one based on following the conditions. 
...
-If CH is ? then the query value X can be either 1 or 0. If X is 1 then the program must print all the odd integers from the list in the order of insertion. Else if 
X is 0 then the program must print all the even integers from the list in the order of insertion. 

Note:
If there is no odd integer or even integer to print from the list, the program must print -1. 
...
Explanation:
After the 1st query, 245 is inserted into the list.
Now the list contains 245. After the 2nd query, 91 is inserted into the list.
Now the list contains 245 91. After the 3rd query, all the odd integers in the list are printed. Hence 245 91 are printed. 
